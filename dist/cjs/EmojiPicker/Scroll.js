...
const Scroll = ({ emojisPerRow, emojiSize, numberScrollRows, focusedEmoji, emojiData, refVirtualList, handleClickInScroll, handleMouseInScroll, itemCount, itemRanges, collapseHeightOnSearch }) => {
    const [arrayOfRows, setArrayOfRows] = (0, react_1.useState)({});
    const infiniteLoaderRef = (0, react_1.useRef)(null);
    const prevFocusedEmoji = (0, react_1.useRef)(null);
    (0, react_1.useEffect)(function resetScrollState() {
        setArrayOfRows({});
    ...
    (0, react_1.useEffect)(function resetRowsWithFocusedEmoji() {
        var _a;
        let prevEmoji = prevFocusedEmoji.current, nextEmoji = focusedEmoji;
        if (prevEmoji == nextEmoji) {
            return;
        }
        let rowsToUpdate = (prevEmoji === null || prevEmoji === void 0 ? void 0 : prevEmoji.row) == (nextEmoji === null || nextEmoji === void 0 ? void 0 : nextEmoji.row) ? [prevEmoji === null || prevEmoji === void 0 ? void 0 : prevEmoji.row] : [prevEmoji === null || prevEmoji === void 0 ? void 0 : prevEmoji.row, nextEmoji === null || nextEmoji === void 0 ? void 0 : nextEmoji.row];
        ...
        while (i <= endIndex) {
            ...
            for (let rowIndex = i; rowIndex < Math.min(range.to, endIndex + 1); rowIndex++) {
                if (rowIndex == range.from) {
                    nextArrayOfRows[rowIndex] = react_1.default.createElement("div", { className: "emoji-picker-category-title", "aria-rowindex": rowIndex + 1, "aria-colindex": 1 }, range.key);
                }
                else {
                    const offset = rowIndex - range.from;
                    const row = emojiData[range.key].slice((offset - 1) * emojisPerRow, offset * emojisPerRow);
                    nextArrayOfRows[rowIndex] = (react_1.default.createElement("ul", { className: "emoji-picker-category-emoji", role: "row", "aria-rowindex": rowIndex + 1 }, row.map((emoji, colIndex) => {
...
const MemoizedScroll = (0, react_1.memo)(Scroll, function ScrollPropsAreEqual(prevProps, nextProps) {
    var _a, _b;
...
const MemoizedRow = (0, react_1.memo)(VirtualRow, function compareRowProps(prevProps, nextProps) {
    ...
    return prevData[prevIndex] === nextData[nextIndex] && !(0, utils_1.shallowDiffer)(prevStyle, nextStyle) && !(0, utils_1.shallowDiffer)(prevRest, nextRest);
});
const LIST_PADDING_SIZE = 9;
const innerElementType = (0, react_1.forwardRef)((_a, ref) => {
